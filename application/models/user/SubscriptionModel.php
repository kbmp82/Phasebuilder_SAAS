...
$expire_time = strtotime($nextBillingDate);
 ...
 public function check_all_subs(){
        ...
        foreach ($query->result() as $row) {
            ...
            $end_date = $row->end_date;
            $status_acct = $row->suspended;
            $suspended_end_date = $row->suspended_end_date;
           // echo "<br>on user: ".$user_id;
            if ($status != 'cancelled') {
               // echo "<br>status: ".$status;
                ...
                $today_time = strtotime(date("Y-m-d\TH:i:s\Z"));
                $expire_time = strtotime($next_bill_date);
                if ($today_time < $expire_time) {

                } else {

                    if ($suspended_end_date != 'none' && $status_acct == 1) {
                        if (strtotime($suspended_end_date) < $today_time) {
                            //suspension allowance time over, delete domains
                            ...
                            foreach ($query->result() as $row) {
                                ...
                                $this->DomainModel->edit_domain($row->domain, 'unassign');
                            }
                        ...
                        if ($status_acct == 0 && $never == 0) {
                            $this->suspendAccount($user_id, 'No subscription found');
