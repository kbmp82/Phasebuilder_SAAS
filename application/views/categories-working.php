      ...
      $.LoadingOverlay("show", {
       ...
       // $("#loader").show();
          ...
          for (var i = 0; i < data.length; i++) {
             // console.log('BRAND IS: '+data[i].brand);
              if(data[i].brand){
...
// keyStart = brand.indexOf(theKeyword) - 1;
 ...
 if(categories != '' && ads_option != '' && site_kwd != ''){
    ...
    buttons: {
	  ...
	  $.LoadingOverlay("show", {
         ...
         // textColor               : "#fff",
          ...
          text: "Testing API Keys..."
           ...
           error: function(xhr, ajaxOptions, thrownError){
            ...
            complete:function(data){
               if(testResult == 1){
                   console.log('access keys are good..building website: '+domain);
                          $.ajax({
	   ...
	   // $("#loader").show();
	    ...
	    console.log(data);
             ...
             if(data.indexOf(".com") > -1 && data.indexOf("failed to deploy wordpress") < 1){
                               // $.LoadingOverlay("hide");
     ...
     successDomain = false;
    console.log('domain not found in data: '+data);
   // successDomain = "bad";
    // window.location.href = "<?php echo base_url() ?>website";
}
			    ...
			    //console.log(data);
                 ...
                 var remainingCats = categoriesArray.length;
                  ...
                  for (var i = 0; i < categoriesArray.length; i++) {
                   ...
                   catObj[catName] = 0;
                    console.log('queueing categories.  catName is: '+catName+' and catObj[catName] is '+catObj[catName]);
                    //console.log(catObj.catName);
                  }
...
function getIncrement(){
     ...
     if(catNum <= categoriesArray.length){
         console.log('adding products '+catNum);
            console.log('total rounds is '+totalRounds);
            console.log('on round '+rounds);
         catName = categoriesArray[catID];
         console.log('category is '+catName+' and catID is: '+catID);
         console.log('catObj[catName] is '+catObj[catName]+' and maxProductsCat is: '+maxProductsCat);
        if(catObj[catName] < maxProductsCat){
          ...
          waitFrame();
            ...
            console.log('cat is at max product of: '+maxProductsCat);
            catID++;
            catNum++;
          remainingCats--;
          addProducts();
         ...
         console.log('done with round '+rounds);
         rounds++;
   ...
   setTimeout(function(){
       ...
       $.ajax({
               ...
               url: "<?php echo base_url() ?>website/update_asg_status",
                 ...
                 data:{ domain: domain },
                 success: function (data) {
                      console.log("asg data is: "+data);
                     if(data.indexOf("message") > -1){
                         var obj = JSON.parse(data);
                        // console.log(data);
                        if(obj['message'].indexOf('No products found') > -1){
                             console.log("No products found in message...message is: "+obj['message']+' setting postsAdded to 0');
                            postsAdded = 0;
                        }else if(obj['message'].indexOf('No new products found') > -1){
                            console.log("No new products found in message...message is: "+obj['message']+' setting postsAdded to 0');
                            postsAdded = 0;
                        }else if(obj['status'] == 'error'){
                            console.log("error with posting products...message is: "+obj['message']+' setting postsAdded to 0');
                            postsAdded = 0;
                        }else{
                           postsAdded = obj['message'].match("Finished. (.*) posts added successfully.");
            postsAdded = postsAdded[1];
            console.log("grabbing posts added....postsAdded = "+postsAdded);
                       }
                        //add total posts to category product count
             if(postsAdded < maxProducts){
                 console.log("postsAdded is less than maxProucts..postsAdded was: "+postsAdded+' and maxProducts is: '+maxProducts);
                //catName = categoriesArray[catID];
                catObj[catName] = maxProductsCat;
            }else{
                console.log('catObj[catName] updated. Total posts BEFOR was: '+catObj[catName]);
                var currentCatNum =  parseInt(catObj[catName]);

                catObj[catName] = currentCatNum + parseInt(postsAdded);
                console.log('Total posts AFTER is: '+catObj[catName]+' and catName is: '+catName);

            }

    // console.log('total posts: '+postsAdded);
    // var category = categoriesArray[catID];
     ...
     waitFrame();
      ...
      console.log('percent is: '+Math.round(percent));
       waitFrame();
             }
...
function updateCategories(){
    console.log('updating catName is: '+catName);
return $.ajax({
                 type: "POST",
                   //	timeout: 40000,
...
function moveOn(data){
    console.log('percent data is: '+data);
                percent = parseInt(data);
                console.log('percent is: '+percent);
               // console.log('products added moving on: '+data);
                catNum++;
                catID++;
       timer = 0;
       waitInterval = 3;
