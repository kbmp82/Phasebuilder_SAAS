...
preg_match_all('/(?:dp|o|gp|-)\/(B[0-9]{2}[0-9A-Z]{7}|[0-9]{9}(?:X|[0-9]))/', $url, $matches);

return substr($matches[0][0], strpos($matches[0][0], "/") + 1);
...
//remove items mathcing string value from array
...
if (isset($proxies)) {  // If the $proxies array contains items, then
 ...
 //Amazon API item lookup
 function call_azon_api_item($azon_asin, $azon_tag, $secret_key, $access_key_id) {
...
//search and replace and save file

	function replace_string_in_file($filename, $string_to_replace, $replace_with, $new_filename){
    	...
    	function call_iwriter($xml){

	   $ch = curl_init();
    curl_setopt( $ch, CURLOPT_URL, 'https://legacy.iwriter.com/api/' );
	...
	//generate random string for database name
   ...
   function get_days_between($newestDate,$oldestDate){
   $newDate = strtotime($newestDate);
   $oldDate = strtotime($oldestDate);
